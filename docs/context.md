...
| **会话类型** | `code` / `design` 会话级不可变抽象（与 mode 正交）：design 写只允许 `<cwd>/game-design/`，内部 4 mode 可互切 | 类型定义 `shared/session-type.ts`；持久化 `electron/agent/session-type-persistence.ts`（sidecar 原子写）；system append 注入 `shared/mode-prompt.ts#buildDesignSessionTypeAppend`；写约束 `electron/agent/session-mode/design-write-guard.ts`（与 plan-mode-guard 并列）；工具派生 `shared/session-type-tools.ts` + `plan-tools.computeModeToolsForType`；`SessionModeController.composeModeAppend` 注入 type-level append |
