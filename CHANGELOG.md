...
升 **minor 线起点**（如 `0.6.0`，patch 为 0 且 minor > 0）时，`prepare-release` 会把上一线全部小版本（`0.5.0`…`0.5.x`）汇总写入本章节；GitHub Release 正文使用该章节（已含汇总则不再重复附加）。补丁版（如 `0.5.6`）不汇总。可用 `npm run release:notes -- 0.6.0` 预览，`--no-aggregate` 关闭自动附加。
...
- **MiniMax 思考强度开关**：供应商预设从单一 M2.7 升级为 M3 / M2.7 / M2.7-highspeed；M3 暴露 off / max 两级强度，上下文窗口 1,000,000；M2.7 仍走 5 档（off / minimal / low / medium / high）。
- **策划会话 system prompt 优化**：明示 8 工具白名单与 write_plan / godot_set_* 黑名单（避免 agent 第一轮误用 bash 读项目外路径）；附加 9 份标准 GDD 骨架（主设计 + 9 个子系统）作为"整理 / 写入 设计文档"任务的隐含模板，禁产 summary / audit / integration-plan 这类自由发挥副产物。
- **X-agent 项目网站上线**：marketing 站点首次发布，独立 repo 单独 build / deploy，不影响 app 包体；README 改写为 Hero + 三段叙事，附 splash 启动屏截图。

### 修复

- **仅展示模型支持的 thinking 级别**：消除 Pi 静默 clamp 后回弹导致的「切到 max 实际是 high」等错觉。
- **node 22 → 24 升级 + WSL cold start 修复**：CI 与本地开发环境对齐 Node 24；Bash 探针在 WSL 冷启动下不再误判。


### 0.5.x 累计变更

以下为 0.5.0 起各小版本面向用户的说明汇总（新→旧）。
...
- **文档 / ADR**：Ask·Plan bash 闸门、goals 路径、密钥与 RPC 握手说明；ADR 摘要记入 docs/context.md
