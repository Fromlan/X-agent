...
基于 [Pi](https://pi.dev) SDK 的 Electron 桌面 Agent 客户端。面向通用编程助手，并对 Godot 编辑器提供 RPC 控制面；支持多工作区 Fleet 切换。
...
| Fleet | 多 `SessionHost` 槽位，顶栏切换「主会话 / 工作区」 |
...
5. 使用 Godot 控制面时：在 **设置 → 工具** 勾选 Godot 工具；项目启用 **X-agent RPC** 并保持桥接已连接
...
Renderer (React) ──window.xAgent──► Preload ──IPC──► Main
                                                      ├─ FleetHostManager → SessionHost(s)
...
- Fleet：worker / reviewer 并行编排策略（多 Host + UI 切换已可用）
