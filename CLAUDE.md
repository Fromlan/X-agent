...
| Addon | `packages/godot-editor-rpc`（0.6.4：endpoint mtime 轮询、`editor_ready` 上报 addonVersion、场景内省 / 调试器 / 资源治理 / 导出 / 配置读写 / 只读内省全套工具；lint 子进程走子线程、只读捕获不切换编辑场景、序列化 5000 节点预算） |
