/**
 * Web mode (the "connecting" state) — ported from thinking-orbs
 * (https://github.com/Jakubantalik/thinking-orbs), MIT © Jakub Antalik.
 * A constellation wires itself: nodes drift on the sphere under slow
 * value noise; any pair closer than `thr` grows an edge, and bright
 * packets run along randomly re-picked node pairs.
 */

import type { Dot, Line, ModeDraw } from "./core";
import {
  fibDir,
  frac,
  hashD,
