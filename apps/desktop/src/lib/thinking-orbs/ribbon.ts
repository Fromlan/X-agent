/**
 * Ribbon mode — ported from thinking-orbs
 * (https://github.com/Jakubantalik/thinking-orbs), MIT © Jakub Antalik.
 * An undulating sash of parallel strands rides a great circle. The same
 * painter also drives the "breathing" (ring) state via the `faceOn`
 * profile flag: a face-on circle whose radius — not its out-of-plane
 * offset — undulates, so it reads as a ring slowly morphing rather than
 * a sash in orbit. `ghostN: 0` opts out of the backdrop ghost sphere.
 */
...
export const drawRibbon: ModeDraw = (ctx, size, t, dark, o) => {
  ...
  for (let w = 0; w < lanes; w++) {
    ...
    for (let k = 0; k < segs; k++) {
      ...
      // outward and pinch inward. Ribbon keeps the original out-of-plane
      // sash wobble.
